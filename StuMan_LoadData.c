...
#include"StuMan_LoadData.h"
#include<stdio.h>

char symbol[2] = {'<' , '>'};//'<'放在一个学生的开头；'>'放在一个学生的最后
char symbool[2] = {'(' , ')'};//'('放在课程中学生学号的开头，')'放在一个课程的结束
char benefit[3] = {'*' , '^' , '+'};//'*'放在essay末尾，'^'放在project末尾，'+'放在award末尾
...
void LoadData(const char *fileDir1,const char *fileDir2){
    ...
    while(examin(p,'<')){
        ...
        //录入素质加分
        //Essay
        Essay *essayh = (Essay*)MALLOC(sizeof(Essay)),
              *essayr = essayh,*essay0=NULL;
        essayh->next = NULL;
        essayh->prev = NULL;
        ...
        //Project
        Project *projecth = (Project*)MALLOC(sizeof(Project)),
                *projectr = projecth,*project0 = NULL;
        projecth->next = NULL;
        projecth->prev = NULL;
        ...
        //Award
        Award *awardh=(Award*)MALLOC(sizeof(Award)),
              *awardr=awardh,*award0=NULL;
        awardh->next = NULL;
        awardh->prev = NULL;
